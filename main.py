...
class VortexTunnelApp(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.NATHAN_NAME, self.MAJID_NAME, self.NATHAN2_NAME = "Majid 2.0", "Majid", "Nathan"
    ...
    def _create_widgets(self):
        ...
        self.ip_entry = ctk.CTkEntry(
            top_frame, placeholder_text="Enter IP to connect...", font=ctk.CTkFont(size=14))
    ...
    def add_file_to_gallery(self, file_id, filename, local_path):
        ...
        self.gallery_frame.update_idletasks()
        print(
            f"DEBUG: File {filename} added to gallery. Total widgets: {len(self.file_gallery_widgets)}")
    ...
    def request_file_download(self, file_id, filename):
        # Changed to white
        self.update_status(f"Requested download of '{filename}'", "white")
    ...
    def load_config_and_history(self):
        ...
        try:
            ...
            if os.path.exists(self.file_gallery_metadata_file):
                print(
                    f"DEBUG: Loading file gallery metadata from {self.file_gallery_metadata_file}")
                ...
                for file_data in loaded_files_from_json:
                    ...
                    if file_id and filename and local_path and os.path.exists(local_path):
                        ...
                        print(
                            f"WARNING: Skipping invalid/missing file from history: {file_data}")
                ...
                for file_id, file_info in self.file_gallery_items_metadata.items():
                    self._create_gallery_item_widget(
                        file_id, file_info["filename"], file_info["local_path"])
                ...
                self._update_drag_drop_label_visibility()
                print(
                    f"DEBUG: Loaded {valid_files_count} files into gallery metadata and displayed.")

        except Exception as e:
            print(f"ERROR: Error loading config or history: {e}")

    def on_closing(self):
        config = {"last_profile": self.profile_menu.get(
        ) if self.my_name else "Select Profile"}
        try:
    ...
    def process_command(self, command_str, from_history=False):
        try:
            ...
            if cmd == "CHAT_MSG":
                ...
                _, file_id = command_str.split(":", 1)
                local_path_to_delete = self.file_gallery_items_metadata.get(
            ...
            print(f"ERROR: Error processing command: {e} -> '{command_str}'")
                ...
                if receiving_file_info:
                    ...
                    if len(buffer) >= filesize:
                        ...
                        with open(filepath, 'wb') as f:
                            f.write(file_data)
                        self.update_status(
                            f"Successfully received {original_filename}", "white")
                        self.after(10, self.add_file_to_gallery,
                    ...
                    if command_str.startswith("FILE_START_TRANSFER"):
                        ...
                        if len(buffer) >= int(filesize_str):
                            ...
                            with open(save_path, 'wb') as f:
                                f.write(file_data)
                            self.update_status(
                                f"Successfully received {save_filename}", "white")
                            self.after(10, self.add_file_to_gallery,
    ...
    def connect_to_peer(self):
        ...
        self.update_status(
            f"Attempting to connect to {peer_ip}:{self.port}...", "orange")
        threading.Thread(target=self._initiate_connection,
                         args=(peer_ip,), daemon=True).start()
    ...
    def profile_selected(self, choice):
        ...
        elif choice == f"I am {self.NATHAN2_NAME}":
            self.my_name = self.NATHAN2_NAME
            self.ip_entry.delete(0, tk.END)
            # Nathan connects to Majid 2.0 by default
            self.ip_entry.insert(0, self.NATHAN_IP)
        else:
            self.my_name = None
            self.ip_entry.delete(0, tk.END)
    ...
    def start_drawing(self, event):
        """Initialize drawing and send mouse position when starting to draw"""
        self.old_x, self.old_y = event.x, event.y
        # Send mouse position when starting to draw
        if self.my_name and self.connected.is_set():
            self.send_command(f"MOUSE_MOVE:{event.x},{event.y},{self.my_name}")

    def draw(self, event):
        # Send mouse position even while drawing to keep the remote mouse marker updated
        if self.my_name and self.connected.is_set():
            self.send_command(f"MOUSE_MOVE:{event.x},{event.y},{self.my_name}")
        ...
        if self.old_x and self.old_y:
            ...
            self.send_command(
                f"DRAW:{self.old_x},{self.old_y},{event.x},{event.y},{self.color},{self.brush_size}")
        self.old_x, self.old_y = event.x, event.y

    def reset_drawing_state(self, event): self.old_x, self.old_y = None, None

    def clear_canvas(self): self.canvas.delete(
        "all"); self.send_command("CLEAR")

    def update_remote_mouse(self, x, y, name):
        if not self.remote_mouse:
            self.remote_mouse = self.canvas.create_oval(
                x-5, y-5, x+5, y+5, fill="red", outline="white")
            self.remote_mouse_label_id = self.canvas.create_text(
                x+10, y-10, anchor="nw", text=name, fill="red", font=("Arial", 10, "bold"))
        else:
    ...
    def delete_file(self, file_id, local_path, is_remote_command):
        print(
            f"DEBUG: Deleting file ID: {file_id}, Path: {local_path}, Remote command: {is_remote_command}")
        if file_id in self.file_gallery_items_metadata:
        ...
        if local_path and os.path.exists(local_path):
            try:
                ...
                if not is_remote_command:
                    messagebox.showerror(
                        "File Deletion Error", f"Could not delete local file: {e}")
        ...
        if not is_remote_command:
            self.update_status(
                f"File deleted and removed from gallery.", "white")

    def notify_user(self):
        # Removed the opacity flashing that was causing issues during drawing
        # Only flash if the window is not already on top
        if not self.master.attributes("-topmost"):
            # Flash the taskbar instead of changing window opacity
            self.master.lift()
